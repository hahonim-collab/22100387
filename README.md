...
## 여러 기기에서 DB를 공유하려면
현재 버전은 브라우저별 로컬 저장 방식입니다. 여러 기기에서 같은 DB를 실시간 공유하려면 Firebase, Supabase 또는 별도 서버가 필요합니다.
...
## DB 업데이트
- `내부품질기한 계산기_김성훈(1).html`의 DATA 배열 기준으로 기본 DB를 교체했습니다.
- 품목 수: 102개
...
## V3 유연한 계산 기준
- 품목별 계산 기준을 `rules[]` 배열로 저장합니다.
- 관리자에서 제조 후, 소분 후, 개봉 후, 해동 후 등 기준을 자유롭게 추가·삭제·이름 변경할 수 있습니다.
- 기존 `portionRule`/`openRule` DB는 자동으로 `소분·제조 후`/`개봉 후` 규칙으로 변환됩니다.
